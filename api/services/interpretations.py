...
def interpret_network_metrics(network_results: Dict[str, Dict[str, float]]) -> Dict[str, str]:
    """
    Interpret network connectivity metrics for time series interactions.
    
    Args:
        network_results (Dict[str, Dict[str, float]]): Dictionary of network metrics
        
    Returns:
        Dict[str, str]: Dictionary of interpretations for network metrics
    """
    interpretations = {}
    
    try:
        # Global network metrics
        if "global" in network_results:
            global_metrics = network_results["global"]
            
            density = global_metrics.get("density", 0.0)
            density_interp = f"""
            Network density is {density:.4f}, indicating {'high' if density > 0.5 else 'moderate' if density > 0.3 else 'low'} overall connectivity. {'This suggests strong information transmission across the system.' if density > 0.5 else 'This suggests partial information transmission across the system.' if density > 0.3 else 'This suggests limited information transmission across the system.'} 
            In market terms, {'this high density indicates a financial system where information and shocks spread quickly across markets, similar to how global markets reacted during the 2008 financial crisis' if density > 0.5 else 'this moderate density indicates some market segmentation with partial transmission of shocks between sectors, as often seen in regional market corrections' if density > 0.3 else 'this low density indicates highly compartmentalized markets where sector-specific events remain isolated, as sometimes observed in commodity market fluctuations that don\'t affect broader equity markets'}. 
            {'Portfolio diversification may be challenging in such a highly connected system as most assets move together during market stress.' if density > 0.5 else 'Some diversification benefits remain possible by selecting assets from less connected portions of the network.' if density > 0.3 else 'Strong diversification opportunities exist as shocks appear to remain contained within specific market segments.'}"""

            interpretations["Network Density"] = density_interp

            clustering = global_metrics.get("clustering", 0.0)
            clustering_interp = (
                f"The clustering coefficient is {clustering:.4f}, suggesting "
                f"{'high' if clustering > 0.6 else 'moderate' if clustering > 0.3 else 'low'} "
                f"tendency for elements to form tightly connected groups. "
                f"{'This indicates the presence of market segments or strong regional effects.' if clustering > 0.6 else
                  'This indicates some market segmentation or regional effects.' if clustering > 0.3 else
                  'This indicates minimal market segmentation or regional effects.'} "
                f"In financial analysis terms, {'the markets form distinct clusters or sectors that exhibit stronger internal connections than external ones, similar to how technology stocks often move together but may diverge from utilities' if clustering > 0.6 else 
                  'there is some evidence of market segmentation, but boundaries between sectors are permeable, allowing cross-sector effects' if clustering > 0.3 else 
                  'market segments show limited cohesion, with assets responding more to global factors than sector-specific dynamics'}. "
                f"{'For portfolio construction, this suggests that diversifying across identified clusters may be more effective than within clusters.' if clustering > 0.6 else 
                  'This market structure allows for moderate diversification benefits through careful asset selection across different market segments.' if clustering > 0.3 else 
                  'The lack of distinct clusters suggests that alternative diversification approaches beyond traditional sector allocation may be necessary.'}"
            )
            interpretations["Clustering"] = clustering_interp
        
        # Node-specific metrics
        for node, metrics in network_results.items():
            if node != "global":
                ...
                if centrality > 0.7:
                    central_interp = (
                        f"The series {node} has high centrality ({centrality:.4f}), "
                        f"indicating it is a central player in the network. It likely "
                        f"has substantial influence on other variables in the system. "
                        f"In market terms, this asset or sector is highly connected to the broader market, "
                        f"similar to major indices like the S&P 500 or systemically important financial institutions. "
                        f"Its movements may provide valuable signals about overall market direction."
                    )
                elif centrality > 0.4:
                    central_interp = (
                        f"The series {node} has moderate centrality ({centrality:.4f}), "
                        f"suggesting it has some influence on the network but is not "
                        f"a dominant variable. "
                        f"In financial markets, this represents a mid-tier asset that both "
                        f"influences and is influenced by broader market conditions. Such assets "
                        f"typically correlate with market indices but may also exhibit independent movements."
                    )
                else:
                    central_interp = (
                        f"The series {node} has low centrality ({centrality:.4f}), "
                        f"suggesting it is relatively peripheral in the network with "
                        f"limited influence on other variables. "
                        f"From an investment perspective, this asset demonstrates independence from "
                        f"broader market factors, potentially offering diversification benefits during "
                        f"market stress periods. It likely moves according to its own specific drivers."
                    )
                
                interpretations[f"{node} Centrality"] = central_interp
                
                if betweenness > 0.6:
                    between_interp = (
                        f"The series {node} has high betweenness ({betweenness:.4f}), "
                        f"suggesting it acts as an important bridge or conduit of "
                        f"information between different parts of the network. "
                        f"In market analysis, this indicates an asset that connects otherwise "
                        f"separated market segments, similar to how certain financial sectors "
                        f"link commodity markets with consumer stocks. Such assets may exhibit "
                        f"early warning signals before contagion spreads across market segments."
                    )
                    interpretations[f"{node} Betweenness"] = between_interp
                
        return interpretations
    except KeyError as e:
        l.error(f"Missing key in network results: {e}")
        return {"error": f"Unable to interpret network metrics due to missing data: {e}"}
    except TypeError as e:
        l.error(f"Type error when processing network metrics: {e}")
        return {"error": "Network results data is not in the expected format"}
    except Exception as e:
        l.error(f"Error interpreting network metrics: {e}")
        return {"error": "Unable to interpret network metrics due to an error."}
